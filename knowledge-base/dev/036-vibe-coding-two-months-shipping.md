---
title: "Vibe Coding 兩月成績單：Notion 建站系統、量化策略與開發工具"
date: "2025-08-22"
tags: 
  - "開發方法論"
  - "全棧開發"
  - "開源專案"
summary: "### Notion 建站系統 **技術棧**: Next.js + Golang **完成度**: 70% **進度**: 比預期慢了一半 **預覽**: https://t.co/gE2zicxD5Z"
curationStatus: "inbox"
usefulnessScore: 76
noveltyScore: 48
evergreenScore: 67
priorityScore: 68
curationNote: "先快速掃摘要與重點段落，再決定要精選或封存。"
source:
  tweetUrl: "https://x.com/wquguru/status/1958853564857819599"
  externalUrl: null
  authorUsername: "wquguru"
---

# Vibe Coding 兩月成績單：Notion 建站系統、量化策略與開發工具

> **來源**: [@wquguru](https://x.com/wquguru/status/1958853564857819599)
>
> **日期**: Fri Aug 22 11:27:38 +0000 2025
>
> **標籤**: `開發方法論` `全棧開發` `開源專案`

---

> **來源**: [@wquguru (WquGuru🦀)](https://twitter.com/wquguru)
> **日期**: 2026-02-18
> **標籤**: `vibe-coding` `notion` `nextjs` `golang` `量化交易` `開源專案`

---

## 兩個月開發成果

空閒時間重度 Vibe coding 兩個月的成績單：

### Notion 建站系統
- **技術棧**: Next.js + Golang
- **完成度**: 70%
- **進度**: 比預期慢了一半
- **預覽**: https://t.co/gE2zicxD5Z

### Vibe coding 方法論
- **技術棧**: Next.js
- **狀態**: 已開源
- **連結**: https://t.co/sG50GUHjp2

### 12 個經典量化策略
- **技術棧**: Next.js
- **連結**: https://t.co/uRTknHqdR0

### Crypto 牛熊監控
- **技術棧**: Python 命令行
- **狀態**: 無預覽
